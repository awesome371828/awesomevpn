...
.stats{display:flex;gap:20px;justify-content:center;flex-wrap:wrap;max-width:940px;margin:0 auto;padding:6vh 24px}
...
.cta{text-align:center;padding:10vh 24px}
...
<div><h4>Контакты</h4><a href="__BOT__" target="_blank">Telegram-бот</a><a href="__SUPPORT_BOT__" target="_blank">Поддержка в Telegram</a></div>
</div>
...
<title>__SITE_NAME__ — приватный интернет в один тап</title>
<meta name="theme-color" content="#07090d">__HEAD__</head><body>
__BG____NAV__
<section class="hero">
...
def build_landing():
    return R(LANDING.replace("__HEAD__", HEAD).replace("__BG__", BG).replace("__NAV__", NAV).replace("__FOOT__", FOOT))

def build_doc(title, updated, body):
    t = DOC(title, updated, body)
    t = t.replace("__HEAD__", HEAD).replace("__BG__", BG).replace("__NAV__", NAV).replace("__FOOT__", FOOT)
    return R(t)
