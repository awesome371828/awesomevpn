...
bot.deleteWebHook().catch(()=>{}); // убираем старый webhook — чинит «не реагирует»
...
const SUPPORT = 'https://t.me/your_support';

const priceFor = m => m * MONTH_PRICE;
...
// ===== Панель пользователя =====
async function panel(chatId, msgId){
  const user = await db.getOrCreateUser(chatId);
  const keys = (await db.getKeys(chatId)).filter(k=>k.status==='active'&&k.expires>Date.now());
  let left=''; if(keys[0]) left=` — ещё ${Math.ceil((keys[0].expires-Date.now())/86400000)} дн.`;

  const text =
    `👋 <b>${chatId===ADMIN?'👑 Админ':'Привет!'}</b>\n`+
    `━━━━━━━━━━━━━━━━\n`+
    `🌐 <b>Awesome VPN</b> работает 👌\n`+
...
bot.onText(/\/start/,async(m)=>{await db.getOrCreateUser(m.chat.id);panel(m.chat.id);});
bot.onText(/\/menu/,async(m)=>{await db.getOrCreateUser(m.chat.id);panel(m.chat.id);});
bot.onText(/\/buy/,async(m)=>{await db.getOrCreateUser(m.chat.id);bot.sendMessage(m.chat.id,'🛒 <b>Выберите тариф:</b>\n\nКаждый месяц +129₽',{parse_mode:'HTML',reply_markup:monthsKB()});});

bot.on('callback_query',async(q)=>{
  const c=q.message.chat.id,m=q.message.message_id,d=q.data;
  await db.getOrCreateUser(c);

  if(d==='menu') return panel(c,m);
  if(d==='buy_menu') return bot.editMessageText('🛒 <b>Выберите на сколько месяцев:</b>\n\nКаждый месяц = +129₽',{chat_id:c,message_id:m,parse_mode:'HTML',reply_markup:monthsKB()});
  ...
  if(d.startsWith('buy_')){
    ...
    return bot.editMessageText(
      ...
      `📌 После подтверждения админом ключ создастся автоматически.`,
  ...
  if(d.startsWith('paid_')){
    ...
    await bot.answerCallbackQuery(q.id,{text:'⏳ Заявка отправлена...'});
    await bot.sendMessage(ADMIN,
      `🔔 <b>Новая оплата!</b>\n\n👤 Юзер: <code>${c}</code>\n🛒 Тариф: ${mo} мес. — ${pr}₽\n\nПодтвердить выдачу ключа?`,
      {parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'✅ Подтвердить',callback_data:`adm_ok_${c}_${mo}`}],[{text:'❌ Отклонить',callback_data:`adm_no_${c}`}]]}});
    await bot.sendMessage(c,`📨 Заявка на оплату <b>${pr}₽</b> отправлена админу. Ожидайте.`,{parse_mode:'HTML'});
  }

  if(d.startsWith('adm_ok_')){
    if(c!==ADMIN)return;
    const [, , uc, mo]=d.split('_');
    try{
...
bot.onText(/\/add (\d+) (\d+)/,async(msg,match)=>{
  ...
  const[,id,amount]=match;await db.getOrCreateUser(+id);await db.addBalance(+id,+amount);
  bot.sendMessage(msg.chat.id,`✅ Выдано ${amount}₽ юзеру ${id}`);
});
bot.onText(/\/remove (\d+) (\d+)/,async(msg,match)=>{
  if(msg.chat.id!==ADMIN)return;
  const[,id,amount]=match;const u=await db.getUser(+id);await db.setBalance(+id,Math.max(0,u.balance-+amount));
  bot.sendMessage(msg.chat.id,`🔻 Баланс юзера ${id} обновлён`);
});
bot.onText(/\/givekey (\d+) (\d+)/,async(msg,match)=>{
  if(msg.chat.id!==ADMIN)return;
  const[,id,days]=match;await db.getOrCreateUser(+id);
  try{
